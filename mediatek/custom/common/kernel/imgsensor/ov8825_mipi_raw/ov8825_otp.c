...
int get_sensor()
{
	    ...
	    {
		    address = 0x3d05 + index*9;
		    module_value = OV8825_read_cmos_sensor(address);
		    module_value1 = OV8825_read_cmos_sensor(address+1);
		    printk("OV8825_read_cmos_sensor(address) %x OV8825_read_cmos_sensor(address+1) %x\n",module_value,module_value1);
		    if(module_value & 0x80)
		    	continue;
		    if(0x01 == (module_value & 0x7f))
		    {
		      	if(0x01 == module_value1)
		      	{
			      	moudle_itype=2;

			      }
			      else if(0x03 == module_value1)
			      {
		    			moudle_itype=3;
		   ...
		   else if(0x02 == (module_value & 0x7f))
		   {
		    	moudle_itype=1;
					if(0x0e == module_value1)
					{
						moudle_itype=4;
					}
					if(0x0a == module_value1)
					{
						moudle_itype=5;
					}
					break;
       ...
       printk("get_sensor moudle_itype = %d index %d\n",moudle_itype,index);
	return moudle_itype;
}
int update_otp_wb(void)
{
	...
	//add by buxiangyu for distinguish diffrent moudle Truly or Liteon
	printk("#########current_otp->customer_id = 0x%x current_otp->lens_id = 0x%x\n",current_otp.customer_id,current_otp.lens_id);
